...
export default function MoviesPage() {
  ...
  const fetchData = useCallback(() => {
    ...
    const url = searchQuery
      ? `/api/movies?search=${encodeURIComponent(searchQuery)}&limit=${ITEMS_PER_PAGE}&offset=${offset}`
      : `/api/movies?limit=${ITEMS_PER_PAGE}&offset=${offset}`;
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="text-center text-gray-500 text-xs pb-4">
              Showing {((currentPage - 1) * ITEMS_PER_PAGE) + 1} - {Math.min(currentPage * ITEMS_PER_PAGE, total)} of {total} movies
            </div>
          </>
        )}
      </div>
