...
export default function TMDBGeneratorPage() {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (data.success) {
        // Store imported items in localStorage
        const existingMovies = JSON.parse(localStorage.getItem('tmdb-movies') || '[]');
        const existingSeries = JSON.parse(localStorage.getItem('tmdb-series') || '[]');
        
        const newMovies = data.imported.filter((item: any) => item.type === 'movie');
        const newSeries = data.imported.filter((item: any) => item.type === 'tv');
        
        localStorage.setItem('tmdb-movies', JSON.stringify([...existingMovies, ...newMovies]));
        localStorage.setItem('tmdb-series', JSON.stringify([...existingSeries, ...newSeries]));
